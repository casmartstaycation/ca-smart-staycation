...
(function(){
  const css=`
    ...
    .unit-lightbox .unit-prev{left:12px}
    .unit-lightbox .unit-next{right:12px}
    ...
    #calendarGrid .calendar-day:hover:not(.disabled):not(.booked){
      transform:scale(1.06);
      border-color:#c9a44c
    }

    #calendarGrid .calendar-day.empty{
      width:42px;
      height:42px;
      min-width:42px;
      min-height:42px;
    ...
    #calendarGrid .calendar-day.disabled{
      color:#aaa;
      background:#f3f3f3;
      border-color:#e5e5e5;
      cursor:not-allowed
    ...
    #calendarGrid .calendar-day.booked{
      background:#d9534f;
      border-color:#d9534f;
      color:#fff;
      cursor:not-allowed;
    ...
    #calendarGrid .calendar-day.checkin,
    #calendarGrid .calendar-day.checkout{
      background:#063b32;
      border-color:#063b32;
      color:#fff;
      font-weight:700
    ...
    #calendarGrid .calendar-day.selected-range{
      background:#d9f2ec;
      border-color:#b9ddd4;
      color:#18332d
    }
    ...
    /* Keep the selected dates clearly circular */
    #calendarGrid .calendar-day.checkin,
    #calendarGrid .calendar-day.checkout,
    #calendarGrid .calendar-day.today{
      border-radius:50%
  ...
  function openPhoto(images,startIndex){
    ...
    m.innerHTML=`
      <button type="button" class="unit-close" aria-label="Close">×</button>
      <button type="button" class="unit-nav unit-prev" aria-label="Previous photo">‹</button>
      <img alt="Accommodation photo">
      <button type="button" class="unit-nav unit-next" aria-label="Next photo">›</button>
    `;
  ...
  function getPanel(){
    ...
    const calendarGroup=
      formGrid?.querySelector('#calendarGrid')
      ?.closest('.form-group');
  ...
  function render(){
    ...
    if(!u){
      panel.innerHTML=
        '<p style="margin:0;color:#888">Select an accommodation to view photos.</p>';
      return;
    }
    ...
    const amenities=
      Array.isArray(u.amenities)
        ?u.amenities.filter(Boolean)
        :[];
      ...
      <div class="unit-gallery">
        ...
        ${
          ...
          <img
            ...
            loading="lazy"
            src="${esc(images[0])}"
            alt="${esc(u.unitName||u.name||'Accommodation')}"
            data-photo="0"
          >
  ...
  async function load(){
    ...
    try{
      ...
      console.warn(
        'Unable to load accommodation photos',
        e
      );
  ...
  const bookedFor=d=>{
    ...
    for(const b of calendarBookings){
      ...
      const a=dOnly(b.checkIn);
      const z=dOnly(b.checkOut);
        ...
        c.onclick=()=>{
          ...
          if(ci){
            ...
            ci.value=
              selectedIn
              ?
              `${selectedIn.getFullYear()}-${String(
                selectedIn.getMonth()+1
              ).padStart(2,'0')}-${String(
                selectedIn.getDate()
              ).padStart(2,'0')}`
              :'';
          ...
          if(co){
            ...
            co.value=
              selectedOut
              ?
              `${selectedOut.getFullYear()}-${String(
                selectedOut.getMonth()+1
              ).padStart(2,'0')}-${String(
                selectedOut.getDate()
              ).padStart(2,'0')}`
              :'';
