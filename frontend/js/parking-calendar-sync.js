...
function isParkingBooking(b){
  ...
  /* Backend stores Parking Only with room:null. If the parking reference is
     not populated in a response, the absence of a room still identifies it. */
...
function apply(){
  const type=String(document.getElementById('bookingType')?.value||'').trim().toLowerCase();
  if(type!=='parking'&&type!=='both')return;
  const grid=document.getElementById('calendarGrid');if(!grid)return;
  const {month:mo,year}=month();
  grid.querySelectorAll('.calendar-day:not(.empty)').forEach(cell=>{
    const n=Number(String(cell.textContent||'').trim());if(!n)return;
    const d=new Date(year,mo,n);d.setHours(0,0,0,0);
    if(bookings.some(b=>isParkingBooking(b)&&overlaps(d,b))){
...
async function refresh(){
  try{
    const r=await fetch(`${API}/bookings?availability=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/json'}});
    const j=await r.json().catch(()=>null);
    const list=Array.isArray(j?.data)?j.data:(Array.isArray(j)?j:[]);
    if(r.ok)bookings=list;
    console.log('PARKING AVAILABILITY:',bookings.filter(isParkingBooking).map(b=>({checkIn:b.checkIn,checkOut:b.checkOut,parkingOnly:b.parkingOnly,parking:b.parking,room:b.room,bookingStatus:b.bookingStatus})));
...
document.addEventListener('DOMContentLoaded',()=>{
  document.getElementById('bookingType')?.addEventListener('change',()=>{refresh();schedule();});
