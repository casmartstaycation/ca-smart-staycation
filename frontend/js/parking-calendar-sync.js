...
function hasParking(b){return b?.parkingOnly===true||String(b?.parkingOnly).toLowerCase()==='true'||!!(b?.parking?._id||b?.parking)||!!(b?.parkingSlot?._id||b?.parkingSlot);}
...
function occupied(d){const t=type();return bookings.some(b=>{if(!active(b)||!overlaps(d,b))return false;const parking=hasParking(b),room=hasRoom(b),bid=String(b.room?._id||b.room||b.unit?._id||b.unit||'');if(t==='parking')return parking; if(t==='both')return parking||(room&&(!roomId()||bid===roomId())); return room&&bid===roomId();});}
...
function apply(){const grid=document.getElementById('calendarGrid');if(!grid)return;const {month,year}=calendarMonth();grid.querySelectorAll('.calendar-day:not(.empty)').forEach(c=>{const n=Number(String(c.textContent||'').trim());if(!n)return;const d=new Date(year,month,n);if(occupied(d)){c.classList.add('booked');c.title=type()==='parking'?'Parking already booked':'Already booked';c.setAttribute('aria-disabled','true');}});}
async function refresh(){try{const r=await fetch(API+'/bookings',{cache:'no-store',headers:{Accept:'application/json'}});const j=await r.json().catch(()=>({}));const data=Array.isArray(j?.data)?j.data:(Array.isArray(j)?j:[]);if(r.ok)bookings=data;}catch(e){console.warn('Booking availability refresh failed',e);}apply();}
function schedule(){setTimeout(apply,0);setTimeout(apply,150);setTimeout(apply,500);setTimeout(refresh,800);}
document.addEventListener('DOMContentLoaded',()=>{const t=document.getElementById('bookingType');t?.addEventListener('change',()=>{refresh();schedule();});document.getElementById('room')?.addEventListener('change',schedule);const grid=document.getElementById('calendarGrid');if(grid)new MutationObserver(()=>schedule()).observe(grid,{childList:true,subtree:true});refresh();schedule();});
