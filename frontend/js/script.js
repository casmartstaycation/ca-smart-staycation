...
function parseBookingDate(value) { if (!value) return null; const s=String(value); if(/^\d{4}-\d{2}-\d{2}/.test(s)){const p=s.slice(0,10).split("-").map(Number);return new Date(p[0],p[1]-1,p[2]);} const d=new Date(value); return Number.isNaN(d.getTime())?null:new Date(d.getFullYear(),d.getMonth(),d.getDate()); }
...
function bookingTypeChanged(){const type=document.getElementById("bookingType").value,room=document.getElementById("room"),guests=document.getElementById("guests"),children=document.getElementById("children"),vehicle=document.getElementById("vehicleSection"),notes=document.getElementById("bookingNotes");const parkingOnly=type==="parking"||type==="parkingonly";if(parkingOnly){room.value="";room.disabled=true;if(guests){guests.value=0;guests.disabled=true;}if(children){children.value=0;children.disabled=true;}if(vehicle)vehicle.style.display="block";if(notes)notes.style.display="none";}else if(type==="both"){room.disabled=false;if(guests)guests.disabled=false;if(children)children.disabled=false;if(vehicle)vehicle.style.display="block";if(notes)notes.style.display="block";}else{room.disabled=false;if(guests)guests.disabled=false;if(children)children.disabled=false;if(vehicle)vehicle.style.display="none";if(notes)notes.style.display="block";}selectedCheckIn=null;selectedCheckOut=null;const a=document.getElementById("checkIn"),b=document.getElementById("checkOut");if(a)a.value="";if(b)b.value="";calculateTotal();loadBookedDates();}

function calculateTotal(){const type=document.getElementById("bookingType").value,roomId=document.getElementById("room")?.value,checkIn=document.getElementById("checkIn")?.value,checkOut=document.getElementById("checkOut")?.value;if(!checkIn||!checkOut){updateSummary(0,0,0,0,0);return;}if(type==="parking"){const s=parseBookingDate(checkIn),e=parseBookingDate(checkOut),n=Math.ceil((e-s)/86400000);if(n<=0){updateSummary(0,0,0,0,0);return;}const p=Number(settings.parkingRate)*n;updateSummary(0,0,p,0,p);return;}if(!roomId){updateSummary(0,0,0,0,0);return;}const room=rooms.find(r=>String(r._id)===String(roomId));if(!room)return;const s=parseBookingDate(checkIn),e=parseBookingDate(checkOut),n=Math.ceil((e-s)/86400000);if(n<=0){updateSummary(0,0,0,0,0);return;}const roomTotal=n*Number(room.price||0),guests=Number(document.getElementById("guests")?.value||1),extra=guests>2?(guests-2)*Number(settings.extraAdultFee||300)*n:0,parkingTotal=type==="both"?Number(settings.parkingRate||0)*n:0,deposit=Number(settings.securityDeposit||0);updateSummary(roomTotal,extra,parkingTotal,deposit,roomTotal+extra+parkingTotal+deposit);}
