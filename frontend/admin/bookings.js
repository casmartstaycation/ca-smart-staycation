...
function renderStats(){ const pending=bookings.filter(b=>b.bookingStatus==="Pending Payment Verification").length; $("totalCount").textContent=bookings.length; $("reservedCount").textContent=bookings.filter(b=>b.bookingStatus==="Reserved").length; $("pendingCount").textContent=pending; $("checkedInCount").textContent=bookings.filter(b=>b.bookingStatus==="Checked In").length; const alert=$("paymentAlert"); if(alert){ alert.hidden=pending===0; $("paymentAlertTitle").textContent=pending===1?"1 Payment Verification Required":`${pending} Payment Verifications Required`; $("paymentAlertText").textContent=pending===1?"A guest has uploaded payment proof and is waiting for your review.":"Guests have uploaded payment proofs and are waiting for your review."; } }
function reviewPendingPayments(){ $("statusFilter").value="Pending Payment Verification"; $("paymentFilter").value=""; renderBookings(); window.scrollTo({top:document.querySelector(".toolbar")?.offsetTop||0,behavior:"smooth"}); }
...
async function protectedAction(path,successMessage){ const token=getAdminToken(); if(!token){showAdminLogin("Please sign in as admin to continue.");return false;} try{ const res=await fetch(`${API}${path}`,{method:"PUT",headers:authHeaders()}); const json=await res.json(); if(res.status===401||res.status===403){showAdminLogin("Your admin session expired. Please sign in again.");return false;} if(!res.ok) throw new Error(json.message||"Action failed."); alert(successMessage||json.message||"Action completed."); await loadBookings(); return true; }catch(err){alert(err.message||"Unable to complete action.");return false;} }
