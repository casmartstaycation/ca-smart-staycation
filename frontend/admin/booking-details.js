...
function adminToken() { return localStorage.getItem("adminToken") || localStorage.getItem("admin_token") || sessionStorage.getItem("adminToken") || sessionStorage.getItem("admin_token") || ""; }
...
window.viewBooking = async function(id) {
    ...
    try {
        const response = await fetch(`${ADMIN_DETAILS_API}/admin/bookings/${encodeURIComponent(id)}/full`, { headers: { Authorization: `Bearer ${adminToken()}` }, cache: "no-store" });
        ...
        document.getElementById("bookingDetails").innerHTML = `
            <div class="detail-grid">
                <div><span>First Name</span><strong>${adminDetailsEscape(booking.firstName || "—")}</strong></div>
                <div><span>Last Name</span><strong>${adminDetailsEscape(booking.lastName || "—")}</strong></div>
                ...
                <div><span>Complete Address</span><strong>${adminDetailsEscape(booking.address || "—")}</strong></div>
                ...
                <div><span>Adults</span><strong>${adminDetailsEscape(booking.adults ?? 0)}</strong></div>
                <div><span>Children</span><strong>${adminDetailsEscape(booking.children ?? 0)}</strong></div>
                <div><span>Payment Status</span><strong>${adminDetailsEscape(booking.paymentStatus || "Pending")}</strong></div>
                <div><span>Subtotal</span><strong>${adminDetailsMoney(booking.subtotalAmount)}</strong></div>
                <div><span>Voucher</span><strong>${adminDetailsEscape(booking.voucherCode || "—")}</strong></div>
                <div><span>Voucher Discount</span><strong>${adminDetailsMoney(booking.voucherDiscountAmount)}</strong></div>
                ...
                <div><span>Payment Deadline</span><strong>${adminDetailsDateTime(booking.paymentDeadline)}</strong></div>
                <div><span>Payment Submitted</span><strong>${adminDetailsDateTime(booking.paymentProofSubmittedAt)}</strong></div>
                <div><span>Payment Verified</span><strong>${adminDetailsDateTime(booking.paymentVerifiedAt)}</strong></div>
                <div><span>Created</span><strong>${adminDetailsDateTime(booking.createdAt)}</strong></div>
                <div><span>Updated</span><strong>${adminDetailsDateTime(booking.updatedAt)}</strong></div>
            </div>
            <div class="notes"><span>Vehicle Information</span><p>Brand: ${adminDetailsEscape(booking.vehicleBrand || "—")} · Model: ${adminDetailsEscape(booking.vehicleModel || "—")} · Color: ${adminDetailsEscape(booking.vehicleColor || "—")} · Plate: ${adminDetailsEscape(booking.plateNumber || "—")}</p></div>
            ${documentLink("Government-Issued ID", uploads.governmentId)}
            ${documentLink("Driver's License", uploads.driversLicense)}
            <div class="notes"><span>Current Payment Proof</span><p>${paymentLink(uploads.paymentProof)}</p></div>
            <div class="notes"><span>Previous Payment Proofs</span><ul>${history}</ul></div>
            <div class="notes"><span>Reschedule Payment Proof</span><p>${paymentLink(uploads.reschedulePaymentProof)}</p></div>
            <div class="notes"><span>Payment Rejection Reason</span><p>${adminDetailsEscape(booking.paymentRejectionReason || "—")}</p></div>
            <div class="notes"><span>Cancellation</span><p>${adminDetailsEscape(booking.cancellationReason || "—")} · Requested: ${adminDetailsDateTime(booking.cancellationRequestedAt)}</p></div>
            <div class="notes"><span>Refund</span><p>Status: ${adminDetailsEscape(booking.refundStatus || "Not Requested")} · Amount: ${adminDetailsMoney(booking.refundAmount)} · Fee: ${adminDetailsMoney(booking.refundFee)} · Processed: ${adminDetailsDateTime(booking.refundProcessedAt)}</p></div>
            <div class="notes"><span>Reschedule History</span><p>${Array.isArray(booking.rescheduleHistory) && booking.rescheduleHistory.length ? booking.rescheduleHistory.map(x => `${adminDetailsDate(x.previousCheckIn)}–${adminDetailsDate(x.previousCheckOut)} → ${adminDetailsDate(x.newCheckIn)}–${adminDetailsDate(x.newCheckOut)} · ${adminDetailsDateTime(x.changedAt)}`).join("<br>") : "No reschedule history."}</p></div>
