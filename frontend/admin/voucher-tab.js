(() => {
  ...
  function addStyles() {
    ...
    s.textContent = `.voucher-management-tab{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:0 14px;border-radius:7px;background:#eef3f0;color:#173f35;border:1px solid #d7e1dc;text-decoration:none;font-weight:700}.voucher-management-tab.active{background:#173f35;color:#fff;border-color:#173f35}#voucherAdminCard.voucher-tab-panel{display:none;margin-top:18px}#voucherAdminCard.voucher-tab-panel.active{display:block}.voucher-delete-btn{background:#fff!important;color:#a1261f!important;border:1px solid #d9b1ad!important}.voucher-delete-btn:hover{background:#fff4f3!important}`;
    document.head.appendChild(s);
  }

  function showTab(active) {
    const card = document.getElementById("voucherAdminCard");
  ...
  async function deleteVoucher(id, code) {
    ...
    try {
      ...
      if (typeof refresh === "function") await refresh(); else document.querySelector(`.voucher-admin-item[data-voucher-id="${CSS.escape(id)}"]`)?.remove();
  ...
  function addDeleteButtons() {
    ...
    list.querySelectorAll(".voucher-admin-item").forEach(row => {
      ...
      if (!actions) return;
      const btn = document.createElement("button"); btn.type = "button"; btn.className = "voucher-delete-btn"; btn.textContent = "Delete";
      btn.addEventListener("click", () => deleteVoucher(id, code)); actions.appendChild(btn);
    });
  }
  ...
  function patchLoader() {
    ...
    async function wrapped() { const result = await original(); setTimeout(addDeleteButtons, 0); return result; }
    wrapped.__patched = true; window.loadAdminVouchers = wrapped;
  }

  function init() {
    addStyles(); addTab(); patchLoader();
    setTimeout(() => { addTab(); patchLoader(); addDeleteButtons(); }, 500);
    setTimeout(() => { addTab(); patchLoader(); addDeleteButtons(); }, 1500);
  }

  document.addEventListener("DOMContentLoaded", init);
  const observer = new MutationObserver(() => { addTab(); patchLoader(); addDeleteButtons(); });
