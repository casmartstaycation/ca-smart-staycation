(() => {
  ...
  // Admin form enhancement. The existing resources.js owns the basic room fields;
  // this feature adds photo upload without changing the existing unit workflow.
  ...
  if (typeof originalOpenForm === "function") {
    window.openForm = function(type, item = null) {
      ...
      const actions = form.querySelector(".form-actions"); form.insertBefore(field, actions || null);
      ...
      form.onsubmit = async e => {
        ...
        const url=`${API}/rooms${editing?.id?`/${editing.id}`:""}`;
        const r=await fetch(url,{method:editing?.id?"PUT":"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token()}`},body:JSON.stringify(body)});
        const j=await r.json(); if(!r.ok) throw Error(j.message||"Unable to save unit.");
        closeModal(); if(typeof load === "function") await load();
      };
    };
