...
router.post("/guest-auth/bookings/:id/reschedule", async (req, res) => {
  try {
    ...
    if (booking.reschedulePending) return res.status(409).json({ success: false, message: "A date-change request is already waiting for convenience-fee payment proof." });
    ...
    if (hasPayment && policy.fee > 0) {
      booking.reschedulePending = true; booking.reschedulePendingCheckIn = newCheckIn; booking.reschedulePendingCheckOut = newCheckOut; booking.rescheduleFee = policy.fee; booking.reschedulePolicyRule = policy.rule; booking.rescheduleRefundAmount = policy.refund; booking.rescheduleRequestedAt = new Date(); booking.reschedulePaymentStatus = "Pending Verification"; await booking.save();
      return res.json({ success: true, paymentRequired: true, inconvenienceFee: policy.fee, refundAmount: policy.refund, refundPolicyRule: policy.rule, newCheckIn, newCheckOut, message: `The new dates are available. Please pay the ₱${policy.fee.toLocaleString()} convenience fee and upload proof of payment before the booking dates are changed.` });
    }
    const previousCheckIn = booking.checkIn; const previousCheckOut = booking.checkOut; booking.checkIn = newCheckIn; booking.checkOut = newCheckOut; booking.rescheduleHistory = booking.rescheduleHistory || []; booking.rescheduleHistory.push({ previousCheckIn, previousCheckOut, newCheckIn, newCheckOut, changedAt: new Date(), policyRule: policy.rule, inconvenienceFee: policy.fee, refundAmount: policy.refund }); await booking.save();
    res.json({ success: true, paymentRequired: false, inconvenienceFee: 0, refundAmount: 0, refundPolicyRule: policy.rule, message: "Booking dates changed successfully. No payment was recorded, so no convenience fee applies.", data: booking });
