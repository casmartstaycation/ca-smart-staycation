...
router.post("/guest-auth/login", async (req, res) => {
  try {
    ...
    // Always authenticate an existing account first. This preserves passwords
    // changed by the guest and avoids incorrectly treating the booking reference
    // as the password for an already-configured account.
    ...
    // For accounts still using their issued/default password, support the
    // booking reference as the temporary credential. Match case-insensitively
    // and ignore surrounding whitespace so copied references work reliably.
    ...
    if (bookingForPassword) {
      account = await GuestAccount.findOne({ bookingReference: bookingForPassword.bookingReference });
      if (!account) {
      ...
      if (account && account.defaultPassword === true && await bcrypt.compare(password, account.passwordHash)) {
        account.lastLoginAt = new Date();
        await account.save();
        const token = jwt.sign({ accountId: account._id.toString(), email: account.email, bookingReference: account.bookingReference }, JWT_SECRET, { expiresIn: "30d" });
        return res.json({ success: true, message: "Login successful.", token, account: publicAccount(account), bookings: await lightweightBookingsForEmail(account.email) });
