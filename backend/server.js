...
app.use('/api',async(req,res,next)=>{try{await connectMongoDB();cleanupTerminalBookingUploads().catch(err=>console.error("UPLOAD CLEANUP TRIGGER ERROR:",err));next();}catch(err){console.error('API DATABASE CONNECTION ERROR:',err.message);res.status(503).json({success:false,message:'Database connection unavailable. Check the MONGODB_URI and MongoDB Atlas network access settings.'});}});
