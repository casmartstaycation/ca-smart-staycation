...
app.use('/api',async(req,res,next)=>{try{await connectMongoDB();const retentionTask=cleanupExpiredTransientUploads().catch(err=>console.error("24H UPLOAD CLEANUP TRIGGER ERROR:",err));const terminalTask=cleanupTerminalBookingUploads().catch(err=>console.error("UPLOAD CLEANUP TRIGGER ERROR:",err));if(process.env.VERCEL)waitUntil(Promise.allSettled([retentionTask,terminalTask]));next();}catch(err){console.error('API DATABASE CONNECTION ERROR:',err.message);res.status(503).json({success:false,message:'Database connection unavailable. Check the MONGODB_URI and MongoDB Atlas network access settings.'});}});
