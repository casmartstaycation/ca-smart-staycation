...
app.post('/api/bookings',async(req,res,next)=>{try{if(!req.body?.parking)return next();const start=new Date(req.body.checkIn),end=new Date(req.body.checkOut);if(Number.isNaN(start.getTime())||Number.isNaN(end.getTime())||end<=start)return next();const settings=await Setting.findOne().select("parkingBlockedDates").lean();const blocked=(settings?.parkingBlockedDates||[]).find(item=>{const key=String(item?.date||"").trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(key))return false;const day=new Date(`${key}T00:00:00.000Z`);return day>=start&&day<end;});if(blocked)return res.status(409).json({success:false,message:`Parking is unavailable on ${blocked.date}${blocked.reason?` (${blocked.reason})`:""}. Please select different dates or choose Accommodation Only.`});next();}catch(err){console.error("PARKING BLOCK DATE CHECK ERROR:",err);res.status(500).json({success:false,message:"Unable to verify parking availability."});}});
...
app.use('/api', require('./routes/icalRoutes'));
