...
app.use(helmet({ contentSecurityPolicy: { directives: { defaultSrc: ["'self'"], baseUri: ["'self'"], connectSrc: ["'self'", "https://www.casmartstaycation.com", "https://casmartstaycation.com", "https://*.vercel.app", "https://ca-smart-staycation-muqd.onrender.com"], fontSrc: ["'self'", "https:", "data:"], formAction: ["'self'"], frameAncestors: ["'self'"], imgSrc: ["'self'", "data:"], scriptSrc: ["'self'", "'unsafe-inline'"], styleSrc: ["'self'", "https:", "'unsafe-inline'"] } } }));
...
app.use(morgan('dev')); app.use(express.json({limit:'10mb'})); app.use(express.urlencoded({extended:true,limit:'10mb'})); app.use('/uploads',express.static(path.join(__dirname,'uploads')));
...
// Keep the public settings endpoint explicit. This avoids a Vercel catch-all/path-normalization mismatch.
app.get('/api/settings', async (req,res)=>{try{let settings=await require('./models/Setting').findOne();if(!settings)settings=await require('./models/Setting').create({});res.json({status:'success',data:settings});}catch(err){console.error('GET SETTINGS ERROR:',err);res.status(500).json({status:'error',message:'Unable to load settings.'});}});
app.put('/api/settings', async (req,res)=>{try{let Setting=require('./models/Setting');let settings=await Setting.findOne();if(!settings)settings=await Setting.create(req.body||{});else{Object.assign(settings,req.body||{});await settings.save();}res.json({status:'success',data:settings});}catch(err){console.error('PUT SETTINGS ERROR:',err);res.status(500).json({status:'error',message:'Unable to save settings.'});}});
...
app.use('/api', require('./routes/parkingRoutes')); app.use('/api',require('./routes/adminRoutes')); app.use('/api',require('./routes/roomRoutes')); app.use('/api',require('./routes/guestRoutes')); app.use('/api',require('./routes/guestFastRoutes')); app.use('/api',require('./routes/guestAuthRoutes'));
